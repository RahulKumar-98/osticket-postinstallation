...
<p>
Next we will configure departments in our Helpdesk. This is significant for providing various levels of access for departments, i.e., SysAdmin ticket visibility or department-specific inquiries (depending upon company structure and delegation of responsibilities). First, we will click on the "Departments" tab under Agent -> Add New Department -> Name Department and configure department settings/ access based on your preferences. Then we will create the department.
</p>
<br />

<img width="977" height="675" alt="5" src="https://github.com/user-attachments/assets/9e83495f-f822-4e3e-833e-e51ea04c19d3" />
<p>
Now we will create a new Team, under the Agents tab we will click on Teams -> "Add new Teams" -> Then we will create the Team. Here, we may create teams dependent upon a specific task or responsibility, i.e Online Banking Team or Maintenance Team.
</p>
<br />
<img width="965" height="685" alt="7" src="https://github.com/user-attachments/assets/b9fcfd60-a3c0-42b6-a0c5-ff6ebe47d349" />

<p>
Now that we've created our relevant teams, we can proceed with the ticket creation process and configure access settings to allow any User to create a ticket without registration on our Helpdesk server. To do this, we will click on the Settings tab within the Admin Panel -> Click on Users -> Uncheck where it says "Require registration and login to create tickets".
</p>
<br />
...
<p>
Next, we will configure agents in our Helpdesk. Click on Agents within the Admin Panel -> Click on Agent (next to Teams)  -> Click "Add New Agent". Here we can add the Agent's contact information, set up their username or password, and lock/ unlock their account.
</p>
<br />

<img width="964" height="1108" alt="image" src="https://github.com/user-attachments/assets/e9aff945-2f67-44a6-8f72-4e7ea1f9525c" />
<p>
Next, we will configure departments in our Helpdesk. Click on Agents within the Admin Panel -> Click on Departments (next to Roles)  -> Click "Add New Department". Here, we can name the Department, edit email preferences, and configure access permissions for the department we created.</p>
<br />
...
<p>
Now we will configure SLAs. Setting the appropriate SLA based on ticket severity is extremely important in the delegation of tasks within a Helpdesk platform like OsTicket. To get started, first we will go back to the Admin Panel -> Click on the Manage Tab -> Click on SLA ->  "Add New SLA Plan".
</p>
<br />
...
<p>
Here we can set various Service Level Agreements (SLA) dependent upon the ticket's severity and priority level. In the above, Sev-A would be considered a top-priority ticket. Ex: CEO's laptop has been hacked or the sales team is unable to access their laptops. We can set a grace period of how many hours we would expect to be needed for the ticket to be resolved. We can also set the schedule for this type of SLA, having it run on a 24/7 schedule, 24/5 schedule, or an 8-5 pm Business hour,s including business holidays schedule.
</p>
<br />
